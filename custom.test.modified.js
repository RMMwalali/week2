...
//following the example of full.test.js
...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const aliceKeypair = new Keypair()//contains public and private keys
    ...
    const recipient = '0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB'
    ...
    expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount.sub(200000000000000000))
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const bobBalace = await token.balanceOf(0)
    expect(bobBalace).to.be.equal(expectedBobBalance)

    const aliceBalance = await token.balanceOf(60000000000000000)
